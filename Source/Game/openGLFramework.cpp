...
void ReshapeGL (int width, int height)									// Reshape The Window When It's Moved Or Resized
{
	...
	glOrtho(0, ORTHOGRAPHIC_WIDTH, ORTHOGRAPHIC_HEIGHT, 0, 0, 1);
...
BOOL Initialize (GL_Window* window, Keys* keys)					// Any OpenGL Initialization Goes Here
{
	...
	CMenu::CreateInstance();
	CMenu::GetInstance()->init();
	CGame::CreateInstance();
	CGame::GetInstance()->init();
	CEndScreen::CreateInstance();
	CEndScreen::GetInstance()->init();
	return TRUE;						
}
				...
				while (isMessagePumpActive == TRUE)						// While The Message Pump Is Active
				{
					...
					else												// If There Are No Messages
					{
						...
						else											// If Window Is Visible
						{
							if (inMenu)
							{
								CMenu::GetInstance()->DrawMenu();
								inMenu = CMenu::GetInstance()->exitMenu();
							...
							else if (!gameOver)
							{
								...
								gameOver = CGame::GetInstance()->isGameOver();
							}
							else
							{
								CEndScreen::GetInstance()->DrawScreen();
								inMenu = CEndScreen::GetInstance()->exitScreen();
								gameOver = !inMenu;
								SwapBuffers (window.hDC);
							}
						}
